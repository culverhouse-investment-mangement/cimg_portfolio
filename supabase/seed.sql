...
-- around a specific injection event. 2020-03-01 is the first
-- fund_snapshot after the injection was fully settled (Feb-2020
-- $645k → Mar-2020 $1.08M), so anchoring here excludes the jump
-- from ALL-range performance. Amount is 0 as a placeholder so the
-- PM can edit it in /admin/cash when the real figure is known.
insert into public.cash_transactions
  (amount, kind, occurred_at, note)
select 0, 'capital_injection', '2020-03-01',
