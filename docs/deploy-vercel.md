...
| `RESEND_API_KEY` | from resend.com | Optional. Without it, `/admin/login` shows the link inline instead of emailing. |
| `RESEND_FROM` | e.g. `CIMG Portfolio <noreply@yourdomain.com>` | Must be a verified sender in Resend. |
...
2. Click **Admin Sign In** → enter your email → either receive an email (if Resend configured) or see an inline link. Click it → land on `/admin`.
